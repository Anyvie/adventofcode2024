...
while ($row = fgets($fd, 10240)) {
    ...
    $r = array_filter(f(0, $numbers, $result));
    if (in_array($result, $r)) $sum1 += $result;

    $r = array_filter(f2(0, $numbers, $result));
...
function f($current, $remaining, $result) {
    if (! count($remaining)) return [$current];

    $next = array_shift($remaining);

    $r1 = f($current+$next, $remaining, $result);
    $r2 = f($current*$next, $remaining, $result);

    return array_merge($r1, $r2);
}


function f2($current, $remaining, $result) {
    if (! count($remaining)) return [$current];

    $next = array_shift($remaining);

    $r1 = f2($current+$next, $remaining, $result);
    $r2 = f2($current*$next, $remaining, $result);
    $r3 = f2($current.$next, $remaining, $result);
